...
class FileCache {
  /// Is a private property that stores the `TodoItem` objects in the cache. It is implemented as a Dictionary where
  /// the key is the ID of the `TodoItem` and the value is the TodoItem itself. It is marked as `private(set)` which
  /// means that it can only be modified within the `FileCache` class, but can be accessed from outside the class.
  private(set) var items: [String: TodoItem] = [:]

  /// This method allows you to add a new `TodoItem` object to the cache.
  /// - Parameter newItem: A TodoItem object which is the item to be added to the cache.
  /// - Returns: A TodoItem object which is the item that was previously associated with the same ID as the new item. If there was no previous item, it returns nil.
  ...
  /// This method allows you to delete a `TodoItem` object from the cache using its ID.
  /// - Parameter id: A String which is the ID of the TodoItem that needs to be deleted from the cache.
  /// - Returns: A TodoItem object which is the item that was deleted from the cache. If there was no item with the given ID, it returns nil.
  ...
  /// This method allows the user to save the `TodoItem` objects stored in the cache to a file on the device.
  /// The method uses JSONSerialization to serialize the `TodoItem` objects in the cache into a JSON array and
  /// writes it to a file in the document directory of the app. The method executes asynchronously on a background
  /// thread to avoid blocking the main thread.
  /// - Parameters:
  ///   - file: A String which is the name of the file to save the `TodoItem` objects.
  ...
  /// This method allows the user to save the `TodoItem` objects stored in the cache to a CSV file on the device.
  /// The method creates a CSV file and writes the `TodoItem` objects in the cache into it. The method executes
  ...
  /// This imethod allows the user to load `TodoItem` objects from a file on the device. The method reads the JSON data
  /// from the specified file and deserializes it into an array of Any objects. It then uses the `parse(json:)` method
  /// of the `TodoItem` class to deserialize each TodoItem object from the array. The deserialized TodoItem objects are
  /// then stored in the cache. The method executes asynchronously on a background thread to avoid blocking the main thread.
  /// - Parameters:
  ///   - file: A String which is the name of the file to load the TodoItem objects.
  ...
  /// This method allows the user to load `TodoItem` objects from a CSV file on the device. The method reads the CSV
  /// data from the specified file and deserializes it into an array of `TodoItem` objects. It then stores the
  /// deserialized `TodoItem` objects in the cache. The method executes asynchronously on a background thread to avoid
    ...
    DispatchQueue.global(qos: .userInitiated).async {
      do {
        ...
        for i in 1 ..< rows.count {
          deserializedItems.append(TodoItem.parse(csv: String(rows[i]))!)
        }
